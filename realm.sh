...
check_dependencies() {
    local dependencies=("wget" "tar" "systemctl" "sed" "grep" "curl" "ss")
    local missing=()
...
add_forward() {
    ...
    while true; do
        read -e -p "本机端口: " lp
        if ! validate_port "$lp" || ! check_port_available "$lp" || check_rule_exists "$lp"; then
            ((attempt++)); [ $attempt -ge 2 ] && return; continue
        fi
        break
    done
